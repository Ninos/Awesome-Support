...
<div class="wpas wpas-submit-ticket">
	<form class="wpas-form" role="form" method="post" action="<?php echo get_permalink( $post->ID ); ?>" id="wpas-new-ticket" enctype="multipart/form-data">
		...
		<div <?php wpas_get_field_container_class( 'form-group wpas_title' ); ?>>
			<label><?php _e( 'Subject', 'wpas' ); ?></label>
			<input class="form-control" name="wpas_title" type="text" <?php wpas_get_field_class( 'wpas_title' ); ?> value="<?php echo wpas_get_field_value( 'wpas_title', true ); ?>" placeholder="<?php echo apply_filters( 'wpas_form_field_placeholder_wpas_title', __( 'What is this about?', 'wpas' ) ); ?>" required>
		</div>
		...
		<div <?php wpas_get_field_container_class( 'form-group wpas_message' ); ?>>
		...
		wpas_make_button( __( 'Submit ticket', 'wpas' ), array( 'name' => 'wpas-submit', 'onsubmit' => __( 'Please Wait...', 'wpas' ), 'class' => 'btn btn-primary' ) );
